...
bool Win32Window::Create(const std::wstring& title,
  ...
  POINT cursor_point;
  const bool has_cursor = GetCursorPos(&cursor_point);
  const POINT target_point = has_cursor
                                 ? cursor_point
  ...
  HWND window = CreateWindow(
      window_class, title.c_str(), WS_OVERLAPPEDWINDOW, left, top, width,
      height,
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_GETMINMAXINFO: {
      ...
      const Size min_size = GetMinimumSize();
      if (min_size.width > 0 && min_size.height > 0) {
        HMONITOR monitor = MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST);
        UINT dpi = FlutterDesktopGetDpiForMonitor(monitor);
        double scale_factor = dpi / 96.0;

        RECT rect = {0, 0, Scale(min_size.width, scale_factor),
                     Scale(min_size.height, scale_factor)};
        AdjustWindowRectExForDpi(&rect, GetWindowLong(hwnd, GWL_STYLE), FALSE,
