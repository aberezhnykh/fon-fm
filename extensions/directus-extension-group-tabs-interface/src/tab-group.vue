...
<template>
	<div>
		<div role="tablist" class="tab-list" @keydown="onKeyDown">
			<tab-button v-for="(field, index) in tabFields" ref="buttons" :key="field.field" :field="field" :fields="fields" :values="values" :validation-errors="validationErrors" :active="index === modelValue" @click="$emit('update:modelValue', index)" />
		</div>
		<slot />
	</div>
</template>
