...
Public Sub CanStartPreparedAfterAd(runtimeState As PlaybackRuntimeState, metaState As PlaybackMetaState, orchestrationState As PlaybackOrchestrationState, transitionInProgress As Boolean) As Boolean
	Return CanUsePreparedTransition(runtimeState, metaState, orchestrationState, transitionInProgress, "ad", "track", "ad_to_track")
End Sub
...
Public Sub PrepareNextPlayable(playQueue As List, runtimeState As PlaybackRuntimeState, metaState As PlaybackMetaState, orchestrationState As PlaybackOrchestrationState, mediaCacheService As MediaCache, playbackFlowState As String, flowPlaying As String, flowIdle As String, flowPreparing As String) As ResumableSub
	...
	Do While playQueue.Size > 0
		...
		If candidateType = "break" Then
			Dim breakCandidate As Map = targetPage.Transition_PeekBreakPreparedCandidate(candidateItem)
			If breakCandidate.IsInitialized And breakCandidate.Size > 0 Then
				candidateItem = breakCandidate
				candidateType = candidateItem.GetDefault("type", "")
	...
	If nextItem.GetDefault("type", "") = "break" Then
		Dim breakPreparedItem As Map = targetPage.Transition_PeekBreakPreparedCandidate(nextItem)
		If breakPreparedItem.IsInitialized And breakPreparedItem.Size > 0 Then
			nextItem = breakPreparedItem
		Else
			Return False
		End If
	End If
	...
	Dim pendingPlayItem As Map = targetPage.Transition_GetDirectorPendingPlayItem(targetAudioKey)
	If pendingPlayItem.IsInitialized And pendingPlayItem.Size > 0 Then
		Trace("prefetch skip reason=target_audio_pending_play audio=" & targetAudioKey & " item=" & DescribeItem(pendingPlayItem))
		Return False
	End If
	Dim pendingPrepareItem As Map = targetPage.Transition_GetDirectorPendingPrepareItem(targetAudioKey)
	If pendingPrepareItem.IsInitialized And pendingPrepareItem.Size > 0 Then
		Trace("prefetch skip reason=target_audio_pending_prepare audio=" & targetAudioKey & " item=" & DescribeItem(pendingPrepareItem))
		Return False
	End If
...
Public Sub PromotePreparedPlayer(runtimeState As PlaybackRuntimeState, dataResolver As DataPlaybackResolver, storage As KeyValueStore, fadeInMs As Int, fadeOutMs As Int, flowTransitioning As String) As Boolean
	...
	If previousAudioKey <> "" And previousAudioKey <> nextAudioKey Then
		Dim boundaryArgs As Map
		boundaryArgs.Initialize
		boundaryArgs.Put("fromAudioKey", previousAudioKey)
		boundaryArgs.Put("toAudioKey", nextAudioKey)
		boundaryArgs.Put("fadeOutMs", fadeOutMs)
		boundaryArgs.Put("fadeInMs", fadeInMs)
		boundaryArgs.Put("reason", "promote_prepared")
		targetPage.Transition_RecordFadeBoundary(boundaryArgs)
