...
Public Sub MarkSlotReady(audioKey As String)
	Dim slot As PlaybackPlayerSlot = GetSlotByAudioKey(audioKey)
	If slot.IsInitialized = False Then Return
	If slot.Role = "pending_play" Or slot.Role = "pending_prepare_music" Or slot.Role = "pending_interrupt" Or slot.Role = "pending_prepare" Then
		slot.SetRoleState(slot.Role, "ready")
		slot.MarkProgress
	End If
End Sub
