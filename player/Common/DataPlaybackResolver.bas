...
Sub Class_Globals
	Private Const SLOT_SWITCH_LEAD_MS As Long = 10000
...
Public Sub ResolveDataSlotAtTicks(offlineData As Map, targetTicks As Long) As Map
	...
	Dim effectiveTicks As Long = targetTicks + SLOT_SWITCH_LEAD_MS
	Dim todayKey As String = FormatIsoDate(effectiveTicks)
	Dim nowMinutes As Int = MinutesOfDayFromTicks(effectiveTicks)
	Dim todayWeekday As String = IsoWeekdayFromTicks(effectiveTicks)
	Dim matchedSlots As List = BuildMatchedSlotsForDate(schedules, todayKey, todayWeekday)
	...
	If selectedSlot.Size = 0 Then
		' После полуночи текущий поток должен продолжаться от последнего слота
		' предыдущего подходящего дня, а не прыгать в первый будущий слот сегодня.
		For dayOffset = 1 To 7
			Dim previousDayTicks As Long = effectiveTicks - dayOffset * DateTime.TicksPerDay
			Dim previousDayKey As String = FormatIsoDate(previousDayTicks)
			Dim previousWeekday As String = IsoWeekdayFromTicks(previousDayTicks)
			Dim previousSlots As List = BuildMatchedSlotsForDate(schedules, previousDayKey, previousWeekday)
			If previousSlots.IsInitialized = False Or previousSlots.Size = 0 Then Continue
			Dim previousSelected As Map
			previousSelected.Initialize
			Dim previousSelectedMinutes As Int = -1
			For Each previousEntryObject As Object In previousSlots
				Dim previousEntry As Map = previousEntryObject
				Dim previousMinutes As Int = previousEntry.GetDefault("slot_minutes", -1)
				If previousMinutes > previousSelectedMinutes Then
					previousSelected = previousEntry
					previousSelectedMinutes = previousMinutes
				End If
			Next
			If previousSelected.IsInitialized And previousSelected.Size > 0 Then
				selectedSlot = previousSelected
				Exit
			End If
		Next
	End If
...
Private Sub BuildMatchedSlotsForDate(schedules As List, targetDateKey As String, targetWeekday As String) As List
	Dim matchedSlots As List
	matchedSlots.Initialize
	If schedules.IsInitialized = False Then Return matchedSlots
	For Each scheduleObject As Object In schedules
		If scheduleObject Is Map Then
			Dim schedule As Map = scheduleObject
			If ScheduleAppliesOnDate(schedule, targetDateKey, targetWeekday) = False Then Continue
...
Public Sub GetPlaylistCacheHeatCountsById(playlistId As String, mediaCacheService As MediaCache) As Map
	Dim result As Map
	result.Initialize
	result.Put("hot", 0)
	result.Put("cold", 0)
	...
	For orderIndex = 0 To order.Size - 1
		Dim trackId As String = order.Get(orderIndex)
		If trackId = "" Then Continue
		Dim trackObject As Object = trackMap.GetDefault(trackId, Null)
		If (trackObject Is Map) = False Then Continue
		...
		track.Put("playlist_id", playlistId)
		track.Put("playlist_track_index", orderIndex)
		If mediaCacheService.HasValidatedLocalMedia(track) = False Then Continue
		If storedTrackIndex < 0 Or orderIndex > storedTrackIndex Then
			hotCount = hotCount + 1
		Else
	...
	result.Put("total", hotCount + coldCount)
	Return result
End Sub
