...
Sub Class_Globals
	...
	Private Const ANDROID_PLAIN_TRACK_CACHE_FORMAT_KEY As String = "android_plain_track_cache_format_v1"
	#If B4J
...
Public Sub Initialize(storageDirValue As String, storageValue As KeyValueStore, targetModuleValue As Object, traceSubNameValue As String, deviceIdValue As String)
	...
	EnsureAndroidPlainTrackCacheFormat
End Sub
...
Public Sub ResolveLocalMediaUri(item As Map) As String
	...
	If itemType = "ad" Then Return BuildFileUri(GetAdsDir, itemId)
...
Public Sub ResolvePlaybackMediaUri(audioKey As String, item As Map) As String
	...
	If HasValidatedLocalMedia(item) = False Then Return ""
	Return ResolveLocalMediaUri(item)
	#Else
	If item.IsInitialized = False Then Return ""
	Dim itemType As String = item.GetDefault("type", "")
	If itemType = "ad" Then Return ResolveLocalMediaUri(item)
	...
	Return BuildFileUri(GetPlaybackTempDir, BuildPlaybackTempTrackFileName(audioKey))
	#End If
End Sub
...
Public Sub CleanupPlaybackTempFiles
	...
	#Else
	DeleteFileIfExists(GetPlaybackTempDir, PRIMARY_TEMP_TRACK_FILE)
	DeleteFileIfExists(GetPlaybackTempDir, SECONDARY_TEMP_TRACK_FILE)
	playbackTempTrackIds.Clear
	#End If
End Sub
...
Private Sub UpdateTrackIndex(item As Map, trackIndex As Map)
	...
	If entry.ContainsKey("saved_at") = False Then entry.Put("saved_at", DateTime.Now)
	entry.Put("last_used_at", DateTime.Now)
	If entry.ContainsKey("play_count") = False Then entry.Put("play_count", 0)
	If entry.ContainsKey("last_played_at") = False Then entry.Put("last_played_at", 0)
...
Public Sub TouchCachedItem(item As Map)
	...
	If itemType = "track" Then
		...
		Dim entry As Map = cachedTrackIndex.GetDefault(itemId, Null)
		If entry.IsInitialized Then
			entry.Put("play_count", entry.GetDefault("play_count", 0) + 1)
			entry.Put("last_played_at", DateTime.Now)
			cachedTrackIndex.Put(itemId, entry)
			cachedTrackIndexDirty = True
		End If
	End If
End Sub

Public Sub GetTrackPlayCount(trackId As String) As Int
	If trackId = "" Then Return 0
	If cachedTrackIndex.IsInitialized = False Then Return 0
	Dim entry As Map = cachedTrackIndex.GetDefault(trackId, Null)
	If entry.IsInitialized = False Then Return 0
	Return entry.GetDefault("play_count", 0)
End Sub

Public Sub HasHotValidatedLocalMedia(item As Map) As Boolean
	If HasValidatedLocalMedia(item) = False Then Return False
	If item.IsInitialized = False Then Return False
	If item.GetDefault("type", "") <> "track" Then Return True
	Dim trackId As String = item.GetDefault("id", "")
	If trackId = "" Then Return False
	Return GetTrackPlayCount(trackId) <= 0
End Sub
...
Private Sub EnsureTrackPlaybackTemp(audioKey As String, item As Map) As Boolean
	...
	Dim tempDir As String = GetPlaybackTempDir
	If playbackTempTrackIds.GetDefault(audioKey, "") = trackId And IsCachedFileUsable(tempDir, tempFileName) Then Return True
	DeleteFileIfExists(tempDir, tempFileName)
	Try
		...
		Dim outputStream As OutputStream = File.OpenOutput(tempDir, tempFileName, False)
		...
		Return IsCachedFileUsable(tempDir, tempFileName)
	Catch
		DeleteFileIfExists(tempDir, tempFileName)
...
Private Sub EnsureAndroidPlainTrackCacheFormat
	#If B4A
	If ToLongDefault(storage.GetDefault(ANDROID_PLAIN_TRACK_CACHE_FORMAT_KEY, 0), 0) = 1 Then Return
	ResetAndroidTrackCacheForPlainPlayback
	storage.Put(ANDROID_PLAIN_TRACK_CACHE_FORMAT_KEY, 1)
	#End If
End Sub

Private Sub ResetAndroidTrackCacheForPlainPlayback
	Trace("Android кэш треков: переход на прямое воспроизведение без temp/xor. Выполняю одноразовый сброс.")
	Try
		If DirectoryExists(GetTracksDir) Then
			Dim listed As List = File.ListFiles(GetTracksDir)
			If listed.IsInitialized Then
				For Each name As String In listed
					If name = "" Then Continue
					If File.IsDirectory(GetTracksDir, name) Then
						DeleteTrackSubdirRecursive(File.Combine(GetTracksDir, name))
					Else
						File.Delete(GetTracksDir, name)
					End If
				Next
			End If
		End If
	Catch
		Trace("Не удалось очистить Android track cache для plain playback. message=" & LastException.Message)
	End Try
	Try
		Dim tempDir As String = GetPlaybackTempDir
		If DirectoryExists(tempDir) Then
			Dim tempFiles As List = File.ListFiles(tempDir)
			If tempFiles.IsInitialized Then
				For Each tempName As String In tempFiles
					If tempName = "" Then Continue
					If File.IsDirectory(tempDir, tempName) Then
						DeleteTrackSubdirRecursive(File.Combine(tempDir, tempName))
					Else
		...
		End If
	Catch
		Trace("Не удалось очистить Android temp playback cache. message=" & LastException.Message)
	End Try
	cachedTrackIndex.Clear
	cachedTrackPlaylistStats.Clear
	cachedTrackIndexDirty = True
	SaveTrackIndex
	CleanupPlaybackTempFiles
	Trace("Android кэш треков сброшен. Ожидаю повторную загрузку треков в raw-формате.")
End Sub
...
Private Sub FindTrackIdByFileName(fileName As String, trackIndex As Map) As String
	For Each key As String In trackIndex.Keys
		...
		End If
		If GetLeafName(fileName) = BuildTrackCacheFileName(key) Then Return key
	Next
	Return ""
End Sub
...
Private Sub FindTrackIdByFileNameWithMetadata(fileName As String, trackIndex As Map, playlistTrackMaps As Map) As String
	...
	If playlistId = "" Then
		...
		Return globalTrackMap.GetDefault(GetLeafName(fileName), "")
	End If
...
Private Sub ResolveRelativeFileUri(baseDir As String, relativeFileName As String) As String
	Return BuildFileUri(ResolveRelativeParentDir(baseDir, relativeFileName), ResolveRelativeLeafName(relativeFileName))
End Sub

Private Sub GetPlaybackTempDir As String
	#If B4J
	Return File.DirTemp
	#Else If B4A
	Dim tempDir As String = File.Combine(storageDir, "temp")
...
Private Sub BuildFileUri(dir As String, fileName As String) As String
	#If B4J
...
Private Sub GetLeafName(fullPath As String) As String
	Dim normalizedName As String = fullPath.Replace("\\", "/")
	Dim slashIndex As Int = normalizedName.LastIndexOf("/")
	If slashIndex < 0 Then Return normalizedName
	Return normalizedName.SubString(slashIndex + 1)
End Sub
