...
Public Sub RunStepAsync As ResumableSub
	...
	TraceCacheHeatState(currentSlot, nextSlot)
	...
	Dim desiredTrackItems As List = BuildDesiredTrackItemsForSlots(currentSlot, nextSlot, currentNeed, nextNeed)
...
Private Sub TraceCacheHeatState(currentSlot As Map, nextSlot As Map)
	Dim currentHeat As Map = CountSlotCacheHeat(currentSlot)
	Dim nextHeat As Map = CountSlotCacheHeat(nextSlot)
	Trace("Кэш: горячий " & currentHeat.GetDefault("hot", 0) & ", холодный " & currentHeat.GetDefault("cold", 0) & _
		"; следующий горячий " & nextHeat.GetDefault("hot", 0) & ", холодный " & nextHeat.GetDefault("cold", 0))
End Sub

Private Sub CountSlotCacheHeat(slot As Map) As Map
	Dim result As Map
	result.Initialize
	result.Put("hot", 0)
	result.Put("cold", 0)
	If slot.IsInitialized = False Or slot.Size = 0 Then Return result
	Dim playlists As List = GetSlotPlaylists(slot)
	If playlists.IsInitialized = False Or playlists.Size = 0 Then Return result
	Dim seenPlaylistIds As Map
	seenPlaylistIds.Initialize
	Dim hotCount As Int = 0
	Dim coldCount As Int = 0
	For Each playlistObject As Object In playlists
		...
		If seenPlaylistIds.ContainsKey(playlistId) Then Continue
		seenPlaylistIds.Put(playlistId, True)
		Dim heat As Map = dataResolverService.GetPlaylistCacheHeatCountsById(playlistId, mediaCacheService)
		hotCount = hotCount + heat.GetDefault("hot", 0)
		coldCount = coldCount + heat.GetDefault("cold", 0)
	Next
	result.Put("hot", hotCount)
	result.Put("cold", coldCount)
	Return result
End Sub
...
Private Sub ResolveSlotNeed(slot As Map, minReserve As Int) As Int
	...
	Dim playableCount As Int = CountHotPlayableTracksInSlot(slot, minReserve)
	Return Max(0, minReserve - playableCount)
End Sub

Private Sub CountHotPlayableTracksInSlot(slot As Map, minReserve As Int) As Int
	...
	For Each playlistObject As Object In playlists
		...
		If seenPlaylistIds.ContainsKey(playlistId) Then Continue
		seenPlaylistIds.Put(playlistId, True)
		totalCount = totalCount + CountUpcomingHotCachedTracksForPlaylist(playlistId, Max(minReserve * 2, perPlaylistMinReserve * 4))
		If totalCount >= minReserve Then Exit
	Next
	
	Return totalCount
End Sub

Private Sub CountUpcomingHotCachedTracksForPlaylist(playlistId As String, lookaheadCount As Int) As Int
	If playlistId = "" Or lookaheadCount <= 0 Then Return 0
	Dim upcomingTracks As List = dataResolverService.GetUpcomingPlaybackTracksById(playlistId, lookaheadCount, mediaCacheService, True)
	...
	For Each trackObject As Object In upcomingTracks
		...
		If mediaCacheService.HasValidatedLocalMedia(track) Then count = count + 1
	Next
	Return count
End Sub

Private Sub BuildDesiredTrackItemsForSlots(currentSlot As Map, nextSlot As Map, currentNeed As Int, nextNeed As Int) As List
	Dim result As List
	result.Initialize
	...
	AddDesiredTrackItemsForSlot(result, seenTrackIds, currentSlot, currentSlotMinReserve, currentNeed, "current")
	AddDesiredTrackItemsForSlot(result, seenTrackIds, nextSlot, nextSlotMinReserve, nextNeed, "next")
	
	Return result
End Sub

Private Sub AddDesiredTrackItemsForSlot(result As List, seenTrackIds As Map, slot As Map, slotReserve As Int, slotNeed As Int, slotLabel As String)
	...
	Dim weightedPlaylistIds As List
	weightedPlaylistIds.Initialize
	Dim playlistOccurrences As Map
	playlistOccurrences.Initialize
	...
	For Each playlistObject As Object In playlists
		...
		If playlistId = "" Then Continue
		weightedPlaylistIds.Add(playlistId)
		playlistOccurrences.Put(playlistId, playlistOccurrences.GetDefault(playlistId, 0) + 1)
		If playlistIds.IndexOf(playlistId) >= 0 Then Continue
		playlistIds.Add(playlistId)
		Dim playlistPlayableCount As Int = CountUpcomingHotCachedTracksForPlaylist(playlistId, Max(slotReserve * 2, perPlaylistMinReserve * 8))
	...
	For Each playlistId As String In playlistIds
		Dim playlistPlayableCount As Int = CountUpcomingHotCachedTracksForPlaylist(playlistId, Max(slotReserve * 2, perPlaylistMinReserve * 8))
		Dim occurrenceWeight As Int = Max(1, playlistOccurrences.GetDefault(playlistId, 1))
		If playlistPlayableCount < perPlaylistMinReserve * occurrenceWeight Then
	...
	For Each playlistObject As Object In playlists
		...
		totalPlayable = totalPlayable + CountUpcomingHotCachedTracksForPlaylist(playlistId, Max(slotReserve * 2, perPlaylistMinReserve * 8))
	Next
	...
	For Each playlistId As String In playlistIds
		...
		Dim playableInPlaylist As Int = CountUpcomingHotCachedTracksForPlaylist(playlistId, Max(slotReserve * 2, perPlaylistMinReserve * 8))
		Dim occurrenceWeight As Int = Max(1, playlistOccurrences.GetDefault(playlistId, 1))
		Dim missingForPlaylist As Int = Max(0, (perPlaylistMinReserve * occurrenceWeight) - playableInPlaylist)
		...
		If addedCount > 0 Then
			Trace("autocache slot fill slot=" & slotLabel & " playlistId=" & playlistId & " added=" & addedCount & " reason=per_playlist_min weight=" & occurrenceWeight)
		End If
	Next
	...
	Do While remainingNeed > 0
		...
		For Each playlistId As String In weightedPlaylistIds
